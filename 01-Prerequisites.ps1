    ...
    if ($Run -or $Install) {
        ...
        if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
            ...
            if (-not $wingetAttempt -and -not (Get-Command winget -ErrorAction SilentlyContinue)) {
                Write-Log -Message "winget not found. Install 'App Installer' from Microsoft Store or https://github.com/microsoft/winget-cli/releases, then re-run this script." -Level ERROR -LogFile $logFile
                Stop-Transcript
                exit 1
            }
            Write-Log -Message "winget is now available." -Level SUCCESS -LogFile $logFile
        }
        # --- Install Node.js, Ollama, Python 3.12+ and other tools
        $tools = @(
            @{Id = "OpenJS.NodeJS"; Name = "Node.js"; Command = "node" },
            @{Id = "Ollama.Ollama"; Name = "Ollama"; Command = "ollama" }
        )
        foreach ($tool in $tools) {
            $setupResults += @{Name = "$($tool.Name) Installation"; Success = (Install-Tool -Id $tool.Id -Name $tool.Name -Command $tool.Command -LogFile $logFile) }
        }
        ...
        if (-not (Test-PythonVersion -Major 3 -Minor 12 -LogFile $logFile)) {
            ...
            if (-not (Test-PythonVersion -Major 3 -Minor 12 -LogFile $logFile)) {
                Write-Log -Message "Python 3.12+ installed but not found in PATH. Close and reopen terminal, then re-run this script." -Level WARN -LogFile $logFile
                Stop-Transcript
                exit 1
            }
            Write-Log -Message "Python 3.12+ installed and verified." -Level SUCCESS -LogFile $logFile
        }
        else { Write-Log -Message "Python 3.12+ detected in system PATH." -Level SUCCESS -LogFile $logFile }
        ...
        foreach ($pkg in $pythonPackages) { $setupResults += @{Name = "$($pkg) Installation"; Success = (Install-PythonPackage -PackageName $pkg -LogFile $logFile) } }
        # --- Install Ollama and sync model
        $setupResults += @{Name = "Ollama Model Sync"; Success = (Sync-JarvisModel -LogFile $logFile) }
    }
