...
# Last edit: 2025-08-10 - Merged 04a/04b/04c into single orchestrator with unchanged end results
...
# -------------------------
# Functions (merged from 04a/04b/04c)
# -------------------------
...
function Test-OllamaSetup {
    ...
    try {
        ...
        if ($response.models.name -contains $defaultModel) { $results += "✅ $defaultModel model available" } else { $results += "❌ $defaultModel model not installed" }
    }
    catch { $results += "❌ Model check failed: $($_.Exception.Message)" }
    $deviceType = $Hardware.OptimalConfig
    $results += if ($deviceType -eq "Qualcomm_NPU" -or $deviceType -eq "Intel_NPU") { "✅ Optimal hardware (NPU) detected" } elseif ($deviceType -eq "NVIDIA_GPU") { "✅ Good hardware (GPU) detected" } else { "⚠️  Suboptimal hardware (CPU) detected" }
...
# -------------------------
# Main execution (combined orchestration)
# -------------------------
try {
    ...
    if ($Install -or $Run) {
        Write-Log -Message "Installing Ollama components and acceleration toolkits..." -Level INFO -LogFile $logFile
        ...
        # Optionally install CUDA if NVIDIA present
        if ($hardware.GPU.Type -eq "NVIDIA") {
            ...
            else { $setupResults += @{Name = "CUDA Toolkit"; Success = $true } }
        }
    }
    ...
    # Additional tuning/optimization steps
    if ($Run -or $Configure) {
    ...
    if (-not $validationSuccess) { Write-Log -Message "Ollama configuration completed with issues - review logs for remediation steps" -Level WARN -LogFile $logFile } else { Write-Log -Message "Ollama configuration completed successfully - all components validated" -Level SUCCESS -LogFile $logFile }
    ...
    if ($validationSuccess) {
        Write-Log -Message "1. For detailed diagnostics: run this script with -Test or use 04b-OllamaDiag.ps1" -Level INFO -LogFile $logFile
        Write-Log -Message "2. For advanced hardware optimization: run this script with -Run (includes tuning)" -Level INFO -LogFile $logFile
    }
    else {
