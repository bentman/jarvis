...
# Last edit: 2025-07-24 - Updated to use centralized model configuration
...
function Test-OllamaModel {
    param([string]$LogFile)
    $requiredModels = @((Get-JarvisModel -LogFile $LogFile))
    $allOk = $true
...
try {
    ...
    if ($Test -or $Run -or $Install) {
        ...
        if ($Install -or $Run -and -not $ollamaInstalled) { $setupResults += @{Name = "Install Ollama"; Success = (Install-Ollama -LogFile $logFile) } }
        elseif ($ollamaInstalled) { $setupResults += @{Name = "Install Ollama"; Success = $true } }
